...
function smm_continue_reading_link() {
	return ' <a href="'. get_permalink() . '">' . __( 'read more', 'smm' ) . '</a>';
}
...
function smm_posted_in() {
	...
	if ( $tag_list ) {
		$posted_in = __( 'This entry was posted in %1$s and tagged %2$s.', 'smm' );
	} elseif ( is_object_in_taxonomy( get_post_type(), 'category' ) ) {
		$posted_in = __( 'This entry was posted in %1$s.', 'smm' );
	} else {
		$posted_in = __( '', 'smm' );
	}
